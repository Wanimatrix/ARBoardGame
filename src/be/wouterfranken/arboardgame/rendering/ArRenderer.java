...
//	int frameWaitAmount = 0;
	...
	private int[] distances = new int[]{20,20,20,20,25,30,35,40,45,50,55,60,65,70,75};
...
//		}
		...
		if(AppConfig.CAMERA_POSE_ESTIMATION && AppConfig.LEGO_TRACKING && AppConfig.LEMMING_RENDERING 
				&& (AppConfig.USE_SAVED_FRAMES || plt == null || plt.getStatus() == AsyncTask.Status.PENDING || plt.getStatus() == AsyncTask.Status.FINISHED)) {
...
//		}
	...
	private class ParallelTask extends AsyncTask<byte[], Void, Void> {
		...
		protected Void doInBackground(byte[]... params) {
			...
			cameraPose.updateCameraPose(colFrameImg, null, getSaveFolderName());
			if(!cameraPose.cameraPoseFound()) {
				if(!AppConfig.USE_SAVED_FRAMES) camera.addCallbackBuffer(params[0]);
//				if(AppConfig.DEBUG_TIMING) Log.d(TAG, "Totaltime in "+(System.nanoTime()-start)/1000000L+"ms");
				TimerManager.stop();
				return null;
			}
			...
			if(AppConfig.LEGO_TRACKING) legoBrick.findLegoBrick(colFrameImg, null, getSaveFolderName());
...
//					lgt.setupFrameTrackingCallback(callback);
					...
					lemmingsGenerator.frameTick(getSaveFolderName());
					if(AppConfig.DEBUG_TIMING) Log.d(TAG, "Lemming frameUpdate in "+(System.nanoTime()-lemmingStart)/1000000L+"ms");
//					callback.trackingDone(LemmingsGenerator.class);
				}
				synchronized (lock) {
